...
// function to check piece exists of opponent
...
// func tion to check whether piece exists or not by square id
...
function giveBishopCaptureIds(id, color){
  ...
  for (let index = 0; index < temp.length; index++) {
    ...
    for (let j = 0; j < arr.length; j++) {
      ...
      let checkPieceResult = checkPieceofopponentonElementNoDom(element);
      if (
        checkPieceResult &&
...
// function to give gighlight Ids for Rook
...
function giveKnightHighlightIds(id) {
  ...
  if (!id) {// to protect it from infinite loop
    return;
  }
...
// function to give highlight Ids ffor queen
...
function giveKingCaptureIds(id, color){
  ...
  result = result.filter(element => {
    if(function giveKingCaptureIds(id, color){
